...
const SaveBtn = ({ data }) => {
    ...
    const handleClick = (e) => {
        ...
        if (allCoins.includes(data.id)) { //if the coin is already saved
            removeCoin(data.id); //clicking on the button again will delete it
        } else {
            saveCoin(data.id)
        }
    }
    ...
    return (
        <>
            <div className='flex flex-col mt-9 border border-gray-100 rounded'>
                {
                    cryptoData ? //if there is crypto data, render the table
                        <table className='w-full table-auto '>
                            <thead className='capitalize text-base text-gray-100 font-medium border-b borer-gray-100'>
                                <tr>
                                    ...
                                    cryptoData.map( (data) => { //map the crypto data to table row
                                        return(
                                            <tr key={data.id} className='text-center text-base border-b border-gray-100 hover:bg-gray-200 last:border-b-0'>
                                                ...
                                                <td className={
                                                    data.price_change_percentage_1h_in_currency > 0 ? 'text-green py-4 px-2' : 'text-red py-4 px-2'
                                                }>
                    ...
                    : null //if there is no crypto data, then null
                }               
            </div>
