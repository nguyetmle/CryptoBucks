import React from 'react'
import { useNavigate } from 'react-router-dom'

const TrendingCoin = ({data}) => {
    let navigate = useNavigate();

    const getCoinDetails = (id) => {
        navigate(`${id}`)
    }


  return (
    <div onClick={() => getCoinDetails(data.item.id)} className='w-[40%] bg-gray-200 mb-12 last:mb-0 rounded-lg p-4 relative cursor-pointer hover:bg-gray-100 hover:bg-opacity-40'>
        {data ?
            <>
                <h3 className='txt-base flex items-center my-0.5'>
                    <span className='text-gray-100'>Name:&nbsp;</span>
                    <span className='text-cyan'> {data.item.name}</span>
                    <img src={data.item.small} alt={data.item.name} className='w-[1.5rem] h-[1.5rem] mx-1.5 rounded-full' />
                </h3>

                <h3 className='txt-base flex items-center my-0.5'>
                    <span className='text-gray-100'>Market Cap Rank:&nbsp;</span>
                    <span className='text-cyan'> {data.item.market_cap_rank}</span>
                </h3>

                <h3 className='txt-base flex items-center my-0.5'>
                    <span className='text-gray-100'>Price (in BTC):&nbsp;</span>
                    <span className='text-cyan'> 
                        {new Intl.NumberFormat("en", {
                            style:'currency',
                            currency: "btc",
                            maximumSignificantDigits: 5
                        }).format(data.item.price_btc)}
                    </span>
                </h3>
                    
                <h3 className='txt-base flex items-center my-0.5'>
                    <span className='text-gray-100'>Score:&nbsp;</span>
                    <span className='text-cyan'> {data.item.score}</span>
                </h3>
                        
                <img src={data.item.large} alt={data.item.name} className='w-[35%] h-auto rounded-full absolute top-2/4 -right-12 -translate-y-2/4' />

            </>

        : null}
    </div>
  )
}

export default TrendingCoin