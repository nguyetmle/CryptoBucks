...
const HighLowIndicator = ({currentPrice, high, low}) => {
    ...
    //calculate the red and green zone of indicator based on high_24h and low_24h price
    useEffect(() => {
        let total = high-low;
        let greenZone = ((high-currentPrice)*100)/total;
        setGreen(Math.ceil(greenZone));  // eslint-disable-next-line react-hooks/exhaustive-deps 
...
const CryptoDetail = () => {
    let { coinId } = useParams(); //get coin id from url
    let { coinData, getCoinData, currency } = useContext(CryptoContext);
    let navigate = useNavigate();

    useLayoutEffect(() => {
        getCoinData(coinId) // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [coinId]) //whenever coinId changes state, call getCoinData again

    const close = () => {
        navigate("..") //navigate to previous route
    }

    return ReactDOM.createPortal(   //whenever create a popup card, it's good to use ReactDOM.createPortal because it doesn't change the page layout/ parent's component
        <div
            onClick={close}
            ...
            <div
                onClick={(e) => e.stopPropagation()} //prevents further propagation of the current event in the capturing and bubbling phases
                className='w-[65%] h-[80%] bg-gray-300 bg-opacity-75 rounded-lg text-white relative'>
                {
                    // check if there is coinData to display info
                    coinData ?
                        <div className='flex items-center justify-between h-full w-full p-4'>
                            ...
                            <div className='flex flex-col w-[45%] h-full pr-2'>
                                <div className='flex w-full items-center'>
                                    ...
                                    <span className='text-sm py-0.5 px-2.5 ml-2 bg-cyan text-cyan bg-opacity-25 rounded uppercase'>{coinData.symbol}</span>
                                </div>
                                ...
                                <div className='flex w-full mt-4 justify-between'>
                                    <div className='flex flex-col'>
                                        ...
                                        {coinData?.links?.official_forum_url[0] && //if there is forum link then display
        ...
        document.getElementById("model") //render the crypto detail 
    )
}
